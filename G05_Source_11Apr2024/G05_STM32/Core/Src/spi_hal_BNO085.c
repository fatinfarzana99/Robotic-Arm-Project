...
TIM_HandleTypeDef *phtim = &htim5; // use timer 5
SPI_HandleTypeDef *phspi = &hspi6;
...
static void hal_init_spi(bool dfu)
{
    ...
    else
    {
        ...
        spi.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_64; // 1.3MHz,  max is 3 MHz
    }
...
{
	if( GPIO_Pin ==  IMU_INT_IRQ_Pin ) //  IMU_INT_Pin
	{
		rxTimestamp_us = timeNowUs();
...
static int dfu_spi_hal_read(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len, uint32_t *t)
{
    ...
    *t = 0;
    ...
    for (int n = 0; n < len; n++)
    {
        rc = HAL_SPI_Receive(phspi, pBuffer+n, 1, 2);
...
static int dfu_spi_hal_write(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len)
{
    ...
    for (int n = 0; n < len; n++)
    {
        rc = HAL_SPI_Transmit(phspi, pBuffer+n, 1, 2);
        delayUs(DFU_BYTE_TIMING_US);
        if (rc != 0)
        {
            break;
